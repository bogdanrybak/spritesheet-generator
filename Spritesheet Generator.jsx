...
function SpriteGenerator() {
    ...
    function saveSizeData(pngPath) {
        ...
        var dataToWrite = pngPath.name + ',';

        if (!dlg.separatePaddingData.value) {
            dataToWrite +=
                parseInt(spriteWidth + paddingLeft + paddingRight) + ',' + parseInt(spriteHeight + paddingTop + paddingBottom) + ','
        } else {
            dataToWrite +=
